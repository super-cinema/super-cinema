...
@CrossOrigin(origins = "*")
public class MovieControllerAg {

    @Autowired
    private MovieFacadeAg movieFacadeAg;

    @PostMapping
    public ResponseEntity saveMovie(@RequestBody MovieDto movieDto) {
        MovieDto movieDtoAdded;
        try {
            movieDtoAdded = movieFacadeAg.saveMovie(movieDto);
        } catch (Exception e) {
    ...
    public ResponseEntity findMovieById(@RequestParam Long movieId){
        MovieDto movieById;
        try{
            movieById = movieFacadeAg.getMovieById(movieId);
    ...
    public MovieDto editMovie(@RequestParam Long id, @RequestBody MovieDto movieDto) {
        return movieFacadeAg.saveEditedMovie(id, movieDto);
    }

    @DeleteMapping
    @RequestMapping(params = "titleToDelete")
    public MovieDto deleteMovie(@RequestParam String titleToDelete) {
        return movieFacadeAg.deleteMovieByTitle(titleToDelete);
    }
    ...
    public ResponseEntity deleteMovie(@RequestParam Long idToDelete){
        try{
            movieFacadeAg.deleteMovie(idToDelete);
